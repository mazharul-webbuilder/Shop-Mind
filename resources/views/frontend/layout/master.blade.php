...
<body class="font-sans antialiased bg-gray-50 text-gray-900">
                ...
                <div class="flex justify-between h-16">
                    <div class="flex">
                        ...
                        <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                            <a href="{{ route('frontend.home') }}" class="inline-flex items-center px-1 pt-1 border-b-2 {{ request()->routeIs('frontend.home') ? 'border-indigo-500 text-gray-900' : 'border-transparent text-gray-500' }} text-sm font-medium leading-5 focus:outline-none focus:border-indigo-700 transition duration-150 ease-in-out">
                                Home
                            </a>
                            <a href="{{ route('frontend.home') }}#shop" class="inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-medium leading-5 text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300 transition duration-150 ease-in-out">
                                Shop
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="hidden sm:flex sm:items-center sm:ml-6 space-x-4">
                        ...
                        <a href="{{ route('frontend.cart.index') }}" id="cart-trigger-btn" class="text-gray-500 hover:text-gray-700 relative">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path>
                            </svg>
                            <span id="cart-badge-count" class="absolute -top-2 -right-2 bg-indigo-600 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">{{ $cartCount ?? 0 }}</span>
                        </a>

                        @auth
                            <div class="ml-3 relative flex items-center space-x-4">
                                <a href="{{ route('frontend.orders.index') }}" class="text-sm font-medium text-gray-500 hover:text-gray-700">My Orders</a>
                        ...
                        @else
                            <div class="ml-3 relative flex items-center space-x-4">
                                <a href="{{ route('frontend.login') }}" class="text-sm font-medium text-gray-500 hover:text-gray-700">Login</a>
                                <a href="{{ route('frontend.register') }}" class="text-sm font-medium text-gray-500 hover:text-gray-700">Register</a>
                            </div>
                        @endauth
                    </div>
                </div>
    ...
    <!-- Mini Cart Drawer -->
    <div id="mini-cart-drawer" class="fixed inset-0 z-50 invisible transition-all duration-300" style="display: none;" aria-labelledby="slide-over-title" role="dialog" aria-modal="true">
        <!-- Backdrop -->
        <div id="mini-cart-backdrop" class="fixed inset-0 bg-gray-500/0 backdrop-blur-none transition-all duration-300 pointer-events-none" aria-hidden="true"></div>

        <div id="mini-cart-wrapper" class="fixed inset-0 overflow-hidden cursor-pointer">
            <div class="absolute inset-0 overflow-hidden">
                <div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10">
                    <!-- Panel -->
                    <div id="mini-cart-panel" class="pointer-events-auto w-screen max-w-md translate-x-full transform transition-all duration-300 ease-in-out bg-white shadow-2xl flex flex-col cursor-default" style="height: 100vh; max-height: 100vh; display: flex; flex-direction: column;">
                        <!-- Header -->
                        <div class="px-6 py-6 border-b border-gray-100 flex items-center justify-between">
                            ...
                            <button type="button" id="close-mini-cart" class="rounded-xl p-2 text-gray-400 hover:text-gray-500 hover:bg-gray-50 transition-colors">
                        ...
                        <div class="px-6 py-4 divide-y divide-gray-100" id="mini-cart-items-container" style="flex: 1 1 0%; min-height: 0px; overflow-y: auto; scrollbar-gutter: stable;">
                        ...
                        <div class="border-t border-gray-100 px-6 py-6 bg-slate-50 space-y-4">
                            ...
                            <div class="grid grid-cols-2 gap-3 pt-2">
                                <a href="{{ route('frontend.cart.index') }}" class="w-full flex items-center justify-center px-4 py-3 border border-gray-200 rounded-xl shadow-sm text-sm font-semibold text-gray-700 bg-white hover:bg-gray-50 transition-all uppercase tracking-wider">
                                    View Cart
                                </a>
                                <a href="{{ route('frontend.checkout.index') }}" class="w-full flex items-center justify-center px-4 py-3 border border-transparent rounded-xl shadow-md text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 transition-all uppercase tracking-wider">
                                    Checkout
                                </a>
                            </div>
                        </div>
